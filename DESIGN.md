...
The terminal and procedural apparatus are one instrument. Terminal input causes visible signal behavior; puzzle progression changes the apparatus structurally. Identity and contact remain behind the release path.

## 2. Color

| Role | Token | Value | Usage |
| --- | --- | --- | --- |
| Surface | `--surface-primary` | `#030806` | Page and canvas ground |
...
- Desktop: terminal occupies the left focal column; the apparatus owns the negative space to its right.
- Mobile: terminal fills the safe viewport; the expensive canvas is replaced by a restrained static field.
...
DPR-capped decorative Canvas 2D apparatus. It reacts to typing and command events, suspends while hidden, and respects reduced motion. It contains no exclusive text or clues.
...
| Ambient | adaptive frame loop | Apparatus idle field |
| Event | command profile | Wake, listen, trace, align, boundary, release |
...
- Outside state: terminal chrome and framing recede instead of adding celebration effects.
